...
!     Set default parameters      
      ...
      ha       = 0.0
      csigm    = 1.0
      tolepsin = 1.0e-5         ! same as default pressure tolerance
      call rzero(ebvec,3)
      call rone(condgrp,16)     ! Warning: never tested!
...
c     Set parameters for the quasi-static MHD
      call finiparser_find(i_out,'qsmhd',ifnd)
      if(ifnd .eq. 1) ifqsmhd = .true.
      if (ifqsmhd) then
         call finiparser_getDbl(d_out,'qsmhd:hartmann',ifnd)
         if(ifnd .eq. 1) ha = d_out
         call finiparser_getDbl(d_out,'qsmhd:ebx',ifnd)
         if(ifnd .eq. 1) ebvec(1) = d_out
         call finiparser_getDbl(d_out,'qsmhd:eby',ifnd)
         if(ifnd .eq. 1) ebvec(2) = d_out
         call finiparser_getDbl(d_out,'qsmhd:ebz',ifnd)
         if(ifnd .eq. 1) ebvec(3) = d_out
         call finiparser_getDbl(d_out,'qsmhd:conductivity',ifnd)
         if(ifnd .eq. 1) csigm = d_out
         call finiparser_getBool(i_out,'qsmhd:residualproj',ifnd)
         if(ifnd .eq. 1 .and. i_out .eq. 1) ipepstrt = 5
         call finiparser_getDbl(d_out,'qsmhd:residualtol',ifnd)
         if(ifnd .eq. 1) tolepsin = d_out
      endif
...
C
      ...
      call bcast(ifqsmhd , lsize)
      call bcast(ipepstrt, isize)
      call bcast(nepprev , isize)
      call bcast(neepslv , isize)
      call bcast(ha      , wdsize)
      call bcast(csigm   , wdsize)
      call bcast(tolepsin, wdsize)
      call bcast(ebvec   , 3*wdsize)
...
c
   ...
   44    format('ERROR: lx1,lx2:',2i4,' lx2 must be lx-2 for IFSPLIT=F')
     ...
     &   'ABORT: Dealiasing space too small; Check lxd,lyd,lzd in SIZE '
      ...
      if (ifqsmhd.and.ifmhd) then
         if (nid.eq.0) write(6,*)
     $        'ABORT: Both quasi-static and full MHD enabled!'
         call exitt
      endif

      if (ifsplit.and.ifqsmhd) then
         if(nid.eq.0) write(6,*) 
     $   'ABORT: No QSMHD support for Pn-Pn'
         call exitt
      endif      
