...
C
      PARAMETER (LTOT2=LX2*LY2*LZ2*LELV)
c
      COMMON /ORTHOV/ RHS(LTOT2,MXPREV), EPRHS(LTOT2,MXPREV)
...
C
      ...
      do ipass=1,npass
c
         if (ifield.eq.ifldmhd.and.ifqsmhd) then
            CALL CDABDTP(Pbrr,EPRHS(1,Kprev),H1,H2,H2INV,INTETYPE)
         else
...
C        Compute part of the norm
         if (ifield.eq.ifldmhd.and.ifqsmhd) then
            Alphad = GLSC2(EPRHS(1,Kprev),Pbrr,NTOT2)
         else
...
C        Gram-Schmidt
         ...
         DO 10 I=1,Kprev1
            if (ifield.eq.ifldmhd.and.ifqsmhd) then
               ALPHA(I) = VLSC2(Pbrr,EPRHS(1,i),NTOT2)
            else
...
C
         DO 20 I=1,Kprev1
            alpham = -alpha(i)
            if (ifield.eq.ifldmhd.and.ifqsmhd) then
               CALL ADD2S2(EPRHS(1,Kprev),EPRHS(1,i),alpham,NTOT2)
            else
...
C
     ...
     $   write(6,*) 'ERROR:  alphad .le. 0 in econj',alphad,Kprev
      ...
      ALPHAN = Alphad
      if (ifield.eq.ifldmhd.and.ifqsmhd) then
         CALL CMULT(EPRHS(1,Kprev),alphan,NTOT2)
      else
...
c     Ohm's equation is independent of the timestep
      if (.not.ifqsmhd.or.ifield.ne.ifldmhd) then
         if (dtlast.ne.dt) then
            ifnewe=.true.
            dtlast=dt
         endif
c     Check for varying conductivity
      elseif (ifqsmhd) then
         nel   = nelfld(ifield)
         nxyz1 = lx1*ly1*lz1
         dsigm = 0.0
         do iel=1,nel
            igrp=igroup(iel)
            if (ifuservp.or.(matype(igrp,ifield).EQ.2)) then
               do i=1,nxyz1
                  dsigm=dsigm+abs(vsprev(i,1,1,iel)-vsigm(i,1,1,iel))
               enddo
            endif
         enddo
         call gop(dsigm,work,'+  ',1)
         if (dsigm.gt.1.0e-6) ifnewe=.true.
      endif
