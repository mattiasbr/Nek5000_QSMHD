...
c     ibc = 2  <==>  Neumann,   
      ...
      do iface=1,2*ldim
         ...
         if (ifqsmhd.and.ifield.eq.ifldmhd) then
            if (cbc(ied,e,ifield).eq.'EC ') ibc = 1
            if (cbc(ied,e,ifield).eq.'EI ') ibc = 2
            if (cbc(ied,e,ifield).eq.'SYM') then
               call chknord(ifalgn,ifnorx,ifnory,ifnorz,ied,e)
               ibc = bsym
               if (ifnorx.and.ebvec(1).eq.0) ibc = 1
               if (ifnory.and.ebvec(2).eq.0) ibc = 1
               if (ifnorz.and.ebvec(3).eq.0) ibc = 1
            endif
         endif
         fbc(iface) = ibc
